...
const CollaborativeEvaluation: React.FC<CollaborativeEvaluationProps> = ({
  ...
  const { data: session, isLoading } = useQuery({
    ...
    queryFn: () => quotesApi.getCollaborativeSession(orderId),
  });

  const { data: teamMembers = [] } = useQuery({
    queryKey: ['team-members', orderId],
    queryFn: () => quotesApi.getTeamMembers(orderId),
  });
  ...
  const submitEvaluationMutation = useMutation({
    mutationFn: (data: any) => quotesApi.submitEvaluation(orderId, selectedQuoteId!, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['collaborative-session', orderId] });
  ...
  const addDiscussionMutation = useMutation({
    mutationFn: (data: any) => quotesApi.addDiscussion(orderId, selectedQuoteId!, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['collaborative-session', orderId] });
      setNewMessage('');
    },
  ...
  const getQuoteEvaluations = (quoteId: string) => {
    return session?.evaluations.filter(e => e.quoteId === quoteId) || [];
  };

  const getQuoteDiscussions = (quoteId: string) => {
    return session?.discussions.filter(d => d.quoteId === quoteId) || [];
  };

  const getAverageRating = (quoteId: string) => {
    const evaluations = getQuoteEvaluations(quoteId);
    if (evaluations.length === 0) return 0;
    return evaluations.reduce((sum, e) => sum + e.rating, 0) / evaluations.length;
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                  {existingEvaluation.pros.map((pro, index) => (
              ...
              <div>
                ...
                <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                  {existingEvaluation.cons.map((con, index) => (
                    <li key={index} className="flex items-start">
