#!/usr/bin/env python3
"""
Edge Case Fix for Smart Matching System
...
class EdgeCaseFix:
    ...
    def perfected_fuzzy_match(self, required: str, available: list) -> float:
    ...
    def enhanced_technical_similarity(self, required: str, available: str) -> float:
        ...
        # Enhanced technical mappings with adjusted scores
        technical_mappings = {
            'cnc machining': {
                'milling': 0.60,      # EDGE CASE FIX: Increased from 0.5
                'turning': 0.60,      # EDGE CASE FIX: Increased from 0.5  
                'precision machining': 0.70,
                'cnc manufacturing': 0.80,
                'machining': 0.40,    # EDGE CASE FIX: Increased from 0.25
                'drilling': 0.35
            },
            'aluminum 6061': {
                'aluminum': 0.60,
            ...
            'aerospace': {
                'defense': 0.50,
                'aviation': 0.70,
                'automotive': 0.30    # EDGE CASE FIX: Some cross-industry overlap
            }
        }
        
        best_similarity = 0.0
        
        for base_term, related_terms in technical_mappings.items():
            if base_term in required:
                for related_term, score in related_terms.items():
                    if related_term in available:
                        best_similarity = max(best_similarity, score)
            elif base_term in available:
                for related_term, score in related_terms.items():
                    if related_term in required:
                        best_similarity = max(best_similarity, score)
    ...
    def perfected_capability_scoring(self, manufacturer_caps: dict, order_reqs: dict) -> float:
        ...
        if base_score >= 0.9:
            ...
            final_score = 0.25 + (base_score - 0.2) * 0.75  # EDGE CASE FIX: Less compression
        else:
