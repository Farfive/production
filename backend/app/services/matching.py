...
class IntelligentMatchingService:
    ...
    def _calculate_capability_score(self, manufacturer: Manufacturer, order: Order) -> float:
        ...
        if 'industry_category' in tech_reqs and 'industries_served' in manufacturer_caps:
            required_industry = order.industry_category or tech_reqs.get('industry_category')
            served_industries = manufacturer_caps['industries_served']
            
            if required_industry:
                industry_score = self._fuzzy_match_list(required_industry, served_industries)
    ...
    def _calculate_distance(self, manufacturer: Manufacturer, order: Order) -> Optional[float]:
        ...
        # For now, return None if coordinates not available
        # In production, you might geocode addresses or use default city coordinates
        if not manufacturer.latitude or not manufacturer.longitude:
            return None
        
        # You would need order location coordinates here
...
MatchingService = IntelligentMatchingService 