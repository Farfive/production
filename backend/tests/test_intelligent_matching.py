...
class TestIntelligentMatchingService:
    ...
    @patch('app.services.intelligent_matching.logger')
    def test_find_best_matches_integration(self, mock_logger, matching_service):
        """Test the main find_best_matches method"""
        ...
        mock_query.all.return_value = [self.sample_manufacturer()]
    ...
    def test_business_filters(self, matching_service, sample_manufacturer, sample_order):
        """Test business logic filters"""
        match_result = matching_service._calculate_comprehensive_score(sample_manufacturer, sample_order)
        matches = [match_result]
        
        # Test with passing filters
        filtered = matching_service._apply_business_filters(matches, sample_order)
        assert len(filtered) == 1
        
        # Test with lead time filter
        sample_order.delivery_deadline = datetime.now() + timedelta(days=5)
        match_result.estimated_lead_time = 10
        filtered = matching_service._apply_business_filters(matches, sample_order)
        assert len(filtered) == 0  # Should be filtered out
    
    def test_fallback_strategy(self, matching_service):
        """Test fallback strategies when no matches found"""
        mock_db = Mock(spec=Session)
        mock_query = Mock()
        mock_db.query.return_value = mock_query
        mock_query.filter.return_value = mock_query
        ...
        matches = matching_service._apply_fallback_strategy(mock_db, order, 5)
        
        assert isinstance(matches, list)
    
    def test_broadcast_functionality(self, matching_service):
        """Test multi-manufacturer broadcast functionality"""
        mock_db = Mock(spec=Session)
        mock_query = Mock()
        mock_db.query.return_value = mock_query
        mock_query.filter.return_value = mock_query
        ...
        # Test broadcast
        result = matching_service.broadcast_to_multiple_manufacturers(
            db=mock_db,
            order=order,
        ...
        assert 'broadcasted_to' in result
        assert 'timestamp' in result
        assert 'expected_responses' in result
...
class TestMatchingAPI:
    """Test API endpoints for intelligent matching"""
    
    def test_matching_request_validation(self):
        """Test request model validation"""
        from app.api.v1.endpoints.intelligent_matching import MatchingRequest
        
        # Valid request
        request = MatchingRequest(order_id=1, max_results=10)
        assert request.order_id == 1
        assert request.max_results == 10
        assert request.enable_fallback == True
        
        # Invalid max_results
        with pytest.raises(ValueError):
            MatchingRequest(order_id=1, max_results=0)
        
        with pytest.raises(ValueError):
            MatchingRequest(order_id=1, max_results=100)
